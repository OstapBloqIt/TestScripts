...
class PatternWindow(Gtk.Window):
    ...
    # -------- drawing primitives --------
    def make_rgb_repeat(self, w):
        g = cairo.LinearGradient(0, 0, w, 0)
        g.add_color_stop_rgb(0.00, 1, 0, 0)   # red
        g.add_color_stop_rgb(1/3, 0, 1, 0)    # green
        g.add_color_stop_rgb(2/3, 0, 0, 1)    # blue
        g.add_color_stop_rgb(1.00, 1, 0, 0)   # red again for seamless repeat
        g.set_extend(cairo.EXTEND_REPEAT)
        return g
    ...
    def draw_checker(self, cr, w, h, off):
        ...
        # integer cell shifts (for color parity), fractional remainders (for smooth slide)
        ix = int(math.floor(ox / cell))
        iy = int(math.floor(oy / cell))
        rx = ox - ix * cell   # == ox % cell but stable
        ry = oy - iy * cell

        cr.set_antialias(cairo.ANTIALIAS_NONE)

        # big margins so edges are covered even at worst remainders
        for y in range(-2*cell, h + 2*cell, cell):
            yy = y - ry
            yi = (y // cell) + iy
            for x in range(-2*cell, w + 2*cell, cell):
                xx = x - rx
                xi = (x // cell) + ix
                black = (xi + yi) & 1
                cr.set_source_rgb(0,0,0) if black else cr.set_source_rgb(1,1,1)
                # snap to ints to avoid fractional edge blending
                cr.rectangle(int(xx), int(yy), cell, cell)
                cr.fill()
                
    def draw_gradient(self, cr, w, h, off):
        self.grad_pat = self.make_rgb_repeat(w)
        pat = self.grad_pat
        m = cairo.Matrix()
        # Negative means it moves left in pattern space, which looks like it slides right to you.
        m.translate(-(off % w), 0)
        pat.set_matrix(m)
        cr.set_source(pat)
        cr.rectangle(0, 0, w, h)
