#!/usr/bin/env python3
# Step 7: Camera with Wayland-compatible layout
...
class CameraWaylandFixed(Gtk.Window):
    def __init__(self):
        super().__init__()
        self.set_title("Step 7: Camera with Wayland Fix")
    ...
    def setup_layout(self):
        ...
        controls_frame.set_size_request(800, 640)
        ...
        # BOTTOM AREA (50% height): Video info (actual video will be in separate window)
        video_frame = Gtk.Frame()
        video_frame.set_size_request(800, 640)
        video_frame.set_shadow_type(Gtk.ShadowType.IN)
        main_vbox.pack_start(video_frame, True, True, 0)

        # Video info label instead of trying to embed
        self.video_info = Gtk.Label()
        self.video_info.set_markup("<span font='16'>Video will appear in separate window when started</span>")
    ...
    def start_camera(self):
        try:
            ...
            # Use waylandsink which creates its own window positioned correctly
