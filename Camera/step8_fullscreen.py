#!/usr/bin/env python3
# Step 8: Fullscreen camera app with positioned video
...
class CameraFullscreen(Gtk.Window):
    def __init__(self):
        super().__init__()
        self.set_title("Step 8: Fullscreen Camera")

        # Start in fullscreen for your 1280x800 rotated display
        self.fullscreen()
        self.set_keep_above(True)  # Keep controls on top
    ...
    def setup_layout(self):
        # Main vertical box - for fullscreen 800x1280 (rotated)
        main_vbox = Gtk.VBox(spacing=0)
        self.add(main_vbox)

        # TOP AREA: Controls (smaller area in fullscreen)
        controls_frame = Gtk.Frame()
        controls_frame.set_size_request(800, 400)  # Smaller controls area
        ...
        # BOTTOM AREA: Video info (larger area for video reference)
        video_frame = Gtk.Frame()
        video_frame.set_size_request(800, 880)  # Rest of screen
        video_frame.set_shadow_type(Gtk.ShadowType.IN)
        main_vbox.pack_start(video_frame, True, True, 0)

        # Video info label
        self.video_info = Gtk.Label()
        self.video_info.set_markup("<span font='16'>Video will appear positioned below these controls</span>")
    ...
    def start_camera(self):
        try:
            ...
            # Try to position video window using environment variables
            # This sets the window position for Wayland
            os.environ['WAYLAND_DISPLAY_WIDTH'] = '800'
            os.environ['WAYLAND_DISPLAY_HEIGHT'] = '880'
            os.environ['WAYLAND_DISPLAY_X'] = '0'
            os.environ['WAYLAND_DISPLAY_Y'] = '400'

            if self.current_format == 'MJPG':
                caps = f"image/jpeg,width={w},height={h},framerate={self.current_fps}/1"
                # Try with window positioning hints
                pipeline_str = f"v4l2src device={self.current_device} ! {caps} ! jpegdec ! videoconvert ! waylandsink window-width=800 window-height=880"
            else:  # YUYV
                caps = f"video/x-raw,format=YUY2,width={w},height={h},framerate={self.current_fps}/1"
                pipeline_str = f"v4l2src device={self.current_device} ! {caps} ! videoconvert ! waylandsink window-width=800 window-height=880"
            ...
            self.video_info.set_markup(f"<span font='16' color='blue'><b>Video playing in window below\n{self.current_format} {w}x{h} @ {self.current_fps}fps</b></span>")
