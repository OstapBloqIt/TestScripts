#!/usr/bin/env python3
# Step 17: Camera with more lower resolution options
...
class CameraMoreResolutions(Gtk.Window):
    def __init__(self):
        super().__init__()
        self.set_title("Step 17: Camera with More Resolutions")
    ...
    def get_video_devices_with_caps(self):
        ...
        for device_path in glob.glob('/dev/video*'):
            try:
                ...
                if result.returncode == 0 and result.stdout.strip():
                    # Parse formats and add extended resolution list
                    if 'H264' in result.stdout:
                        device_info['formats'] = ['H264']
                        # Extended resolutions for H.264 - more lower resolutions
                        device_info['resolutions'] = [
                            (160, 120),   # QQVGA
                            (320, 240),   # QVGA
                            (352, 288),   # CIF
                            (480, 360),   # 360p
                        ...
                        # Extended resolutions for MJPG/YUYV - more lower resolutions
                        device_info['resolutions'] = [
                            (160, 120),   # QQVGA
                            (176, 144),   # QCIF
                            (320, 240),   # QVGA
        ...
        # Fallback if no devices found - with extended resolutions
        if not devices:
            devices = [{
                'path': '/dev/video2',
                'formats': ['MJPG', 'YUYV'],
    ...
    def setup_layout(self):
        ...
        self.status_label.set_markup("<span font='18'><b>Camera with Extended Resolutions</b></span>")
        ...
        self.res_btn = Gtk.Button(label="320x240")  # Start with a lower resolution
        ...
        # Video info label with resolution list
        ...
        # Initialize current values - start with lower resolution
        self.current_format = self.current_device_info['formats'][0] if self.current_device_info else 'MJPG'
        self.current_resolution = (320, 240)  # Start with QVGA

    def update_device_info(self):
        """Update the device info display with resolution list"""
        if self.current_device_info:
            resolutions = self.current_device_info['resolutions']
            res_list = []

            # Group resolutions by type for better display
            ...
            info_text += f"<b>Available Resolutions ({len(resolutions)}):</b>\n"

            # Display resolutions in rows of 3
            for i in range(0, len(res_list), 3):
                row = res_list[i:i+3]
                info_text += f"{' | '.join(row)}\n"

            info_text += f"\n<b>Video displayed in 640x480 window</b>\n"
    ...
    def cycle_device(self, btn):
        try:
            ...
            # Reset to first resolution for new device
            self.current_resolution = self.current_device_info['resolutions'][0]
            w, h = self.current_resolution
    ...
    def start_camera(self):
        try:
            ...
            info_text = f"<span font='14' color='blue'><b>Video playing in {video_w}x{video_h} window\n"
