#!/usr/bin/env python3
# Step 4: Camera with format and resolution controls
...
class CameraWithFormatResolution(Gtk.Window):
    def __init__(self):
        super().__init__()
        self.set_title("Step 4: Camera with Format & Resolution")
    ...
    def start_camera(self):
        try:
            ...
            if self.current_format == 'MJPG':
                caps = f"image/jpeg,width={w},height={h}"
            ...
            self.status_label.set_text(f"Running: {self.current_device} {self.current_format} {w}x{h}")
...
if __name__ == "__main__":
    app = CameraWithFormatResolution()
    Gtk.main()