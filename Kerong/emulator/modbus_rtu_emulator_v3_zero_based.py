#!/usr/bin/env python3
"""
Modbus RTU Slave Emulator for Windows - Version 3 (CU48 Protocol) - Zero-based
- Devices can start at unit id 0x00 (zero-based addressing).
- Optionally treat 0x00 as broadcast (no reply) to conform with Modbus spec.
Derived from the "Fixed" build that includes robust framing and consistent responses.
"""
...
class ErrorDetail:
    ...
    def format_detailed(self) -> str:
        ...
        if len(self.frame) >= 2:
            ...
            }.get(func_code & 0x7F, "Unknown/Invalid")
...
class CU48Device:
    def __init__(self, device_id: int):
        ...
        # Uniform defaults for consistency
        self.holding_registers[0x03] = 550  # unlock duration ms
    ...
    def read_discrete_inputs(self, start_addr: int, count: int) -> bytes:
        ...
        for b in range(byte_count):
            ...
            for bit in range(8):
                idx = start_addr + b*8 + bit
                if idx < start_addr + count and idx < len(self.discrete_inputs) and self.discrete_inputs[idx]:
                    v |= (1 << bit)
            res.append(v)
        return bytes(res)
    ...
    def write_single_coil(self, addr: int, value: int) -> Tuple[bytes, Optional[str]]:
        ...
        if value == 0xFF00:
            ...
            op = f"LOCK #{addr+1} CLOSED"
        elif value == 0x0000:
            self.coils[addr] = False
            op = f"LOCK #{addr+1} UNLOCKED"
        else:
        ...
        for i in range(count):
            ...
            if b < len(values):
                ...
                if new_state != old:
                    ...
                    ops.append(f"Lock #{start_addr + i + 1} {status}")
    ...
    def build_response(self, device_addr: int, function_code: int, payload: bytes) -> bytes:
        if len(payload) == 2 and (payload[0] & 0x80):
            resp = bytes([device_addr]) + payload
        else:
            resp = bytes([device_addr, function_code]) + payload
    ...
    def process_request(self, request: bytes) -> Tuple[Optional[bytes], Optional[str]]:
        ...
        try:
            if fc == 0x01:
                ...
                return (self.build_response(device_addr, fc, self.read_coils(sa, cnt)), None)
            if fc == 0x02:
                sa = struct.unpack('>H', request[2:4])[0]
                cnt = struct.unpack('>H', request[4:6])[0]
                return (self.build_response(device_addr, fc, self.read_discrete_inputs(sa, cnt)), None)
            if fc == 0x03:
                sa = struct.unpack('>H', request[2:4])[0]
                cnt = struct.unpack('>H', request[4:6])[0]
                return (self.build_response(device_addr, fc, self.read_holding_registers(sa, cnt)), None)
            if fc == 0x04:
                sa = struct.unpack('>H', request[2:4])[0]
                cnt = struct.unpack('>H', request[4:6])[0]
                return (self.build_response(device_addr, fc, self.read_input_registers(sa, cnt)), None)
            if fc == 0x05:
                a = struct.unpack('>H', request[2:4])[0]
                v = struct.unpack('>H', request[4:6])[0]
                payload, op = self.write_single_coil(a, v)
                return (self.build_response(device_addr, fc, payload), op)
            if fc == 0x06:
                a = struct.unpack('>H', request[2:4])[0]
                v = struct.unpack('>H', request[4:6])[0]
                return (self.build_response(device_addr, fc, self.write_single_register(a, v)), None)
            if fc == 0x0F:
                sa = struct.unpack('>H', request[2:4])[0]
                cnt = struct.unpack('>H', request[4:6])[0]
                bc = request[6]
            ...
            if fc == 0x10:
                ...
                bc = request[6]
                vals = request[7:7+bc]
                return (self.build_response(device_addr, fc, self.write_multiple_registers(sa, cnt, vals)), None)
            # Unsupported
            payload = bytes([fc | 0x80, 0x01])
            return (self.build_response(device_addr, fc, payload), None)
        except Exception:
...
class ModbusRTUEmulator:
    def __init__(self, num_devices: int, zero_based: bool, zero_is_broadcast: bool):
        self.num_devices = num_devices
        self.zero_based = zero_based
        self.zero_is_broadcast = zero_is_broadcast

        if zero_based:
            # 0x00..(N-1)
            self.devices = {i: CU48Device(i) for i in range(0, num_devices)}
        else:
            # 0x01..N
    ...
    def _process_frame(self, frame: bytes):
        ...
        # Broadcast handling for 0x00 if enabled
        if self.zero_is_broadcast and device_addr == 0x00:
            self._log_raw_response(None, "No response - 0x00 treated as broadcast", timestamp)
            return
...
class EmulatorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Modbus RTU Slave Emulator V3 - CU48 Protocol (Zero-based)")
    ...
    def _create_widgets(self):
        ...
        # Zero-based and broadcast toggles
        self.zero_based_var = tk.BooleanVar(value=True)
        self.zero_is_broadcast_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(config_frame, text="Start at 0x00 (zero-based unit IDs)", variable=self.zero_based_var).grid(row=1, column=3, sticky=tk.W, padx=5, pady=5)
        ttk.Checkbutton(config_frame, text="Treat 0x00 as broadcast (no reply)", variable=self.zero_is_broadcast_var).grid(row=1, column=4, sticky=tk.W, padx=5, pady=5)
        ...
        self.selected_device_var = tk.IntVar(value=0)
        self.device_selector = ttk.Spinbox(device_select_frame, from_=0, to=255, textvariable=self.selected_device_var, width=5, command=self._update_lock_display)
    ...
    def _close_all_locks(self):
        ...
        device_id = self.selected_device_var.get()
        if device_id not in self.emulator.devices: return
        dev = self.emulator.devices[device_id]; dev.coils = [True]*48
        self._update_lock_display(); self._log(f"Device {device_id}: All locks CLOSED/LOCKED")
    ...
    def _unlock_all_locks(self):
        ...
        device_id = self.selected_device_var.get()
        if device_id not in self.emulator.devices: return
        dev = self.emulator.devices[device_id]; dev.coils = [False]*48
        self._update_lock_display(); self._log(f"Device {device_id}: All locks UNLOCKED")

    def _on_lock_state_change(self, device_id: int, operation: str):
        self._log(f"Device {device_id}: {operation}")
        if self.emulator and device_id == self.selected_device_var.get():
    ...
    def _start_emulator(self):
        ...
        zero_based = self.zero_based_var.get()
        zero_is_broadcast = self.zero_is_broadcast_var.get()
        try:
            self.emulator = ModbusRTUEmulator(num_devices, zero_based, zero_is_broadcast)
            ...
            addr_range = (f"0x00..0x{num_devices-1:02X}" if zero_based else f"0x01..0x{num_devices:02X}")
            flags = []
            if zero_based: flags.append("zero-based IDs")
            if zero_is_broadcast: flags.append("0x00 as broadcast")
            self.status_label.config(text=f"Status: Running on {port} @ {baudrate} baud, devices {addr_range} ({', '.join(flags) if flags else 'standard'})",
                                     foreground="green")
            self.start_btn.config(state=tk.DISABLED); self.stop_btn.config(state=tk.NORMAL)
            self._log(f"CU48 Emulator V3 Zero-based started: {port} @ {baudrate} baud")
            self._log(f"Emulating devices {addr_range}")
            if zero_is_broadcast:
                self._log("0x00 is treated as broadcast — emulator will NOT reply to address 0x00")
            else:
                self._log("0x00 is treated as a normal unit ID — emulator WILL reply to address 0x00")
            self._update_lock_display(); self._update_statistics()
        except Exception as e:
            messagebox.showerror("Error", str(e))

    def _stop_emulator(self):
        if self.emulator: self.emulator.stop(); self._log("\n" + self.emulator.stats.get_summary()); self.emulator = None
