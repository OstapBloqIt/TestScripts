...
Adds explicit handling for Read Coils 'count==0':
- Default (spec): return exception 0x03 (Illegal data value) when count==0
- Optional CU48 compatibility: treat count==0 as 48 (read all locks)
Also keeps robust frame splitting and consistent response wrapping.
"""
...
class ErrorDetail:
    ...
    def format_detailed(self) -> str:
        ...
        lines.append(""); lines.append("Frame Analysis:")
        if len(self.frame) >= 1: lines.append(f"  Device Address: 0x{self.frame[0]:02X} ({self.frame[0]})")
        if len(self.frame) >= 2:
            fc = self.frame[1]
            fname = {
            ...
            }.get(fc & 0x7F, f"Unknown/Invalid")
            lines.append(f"  Function Code:  0x{fc:02X} ({fname})")
        if len(self.frame) >= 4:
            if self.error_type == "CRC":
                lines.append(f"  Received CRC:   {self.frame[-2:].hex().upper()}")
                exp = self.expected_value.hex().upper() if self.expected_value else "????"
                lines.append(f"  Expected CRC:   {exp}")
            else:
                lines.append(f"  CRC-16:         {self.frame[-2:].hex().upper()}")
        lines.append("=" * 70)
        return "\n".join(lines)
...
class Statistics:
    ...
    def get_recent_errors_summary(self) -> str:
        ...
        for error in reversed(self.recent_errors):
            lines.append(error.format_detailed()); lines.append("")
        return "\n".join(lines)
...
class CU48Device:
    ...
    def read_coils(self, start_addr: int, count: int) -> bytes:
        ...
        if start_addr >= 48 or start_addr + count > 48:
            return bytes([0x01 | 0x80, 0x02])  # Illegal data address
        ...
        for b in range(byte_count):
            ...
            for bit in range(8):
                ...
                if idx < start_addr + count and idx < 48 and self.coils[idx]:
                    v |= (1 << bit)
            res.append(v)
        return bytes(res)
    ...
    def write_single_coil(self, addr: int, value: int) -> Tuple[bytes, Optional[str]]:
        ...
        if value == 0xFF00:
            self.coils[addr] = True; op = f"LOCK #{addr+1} CLOSED"
        elif value == 0x0000:
            self.coils[addr] = False; op = f"LOCK #{addr+1} UNLOCKED"
        else:
    ...
    def write_multiple_coils(self, start_addr: int, count: int, values: bytes) -> Tuple[bytes, Optional[str]]:
        ...
        for i in range(count):
            b = i // 8; bit = i % 8
            if b < len(values):
                new_state = bool(values[b] & (1 << bit))
                old = self.coils[start_addr + i]
                self.coils[start_addr + i] = new_state
                if new_state != old:
                    ops.append(f"Lock #{start_addr + i + 1} {'CLOSED' if new_state else 'UNLOCKED'}")
...
class CommandLog:
    ...
    def format_log(self) -> str:
        ...
        if self.raw_request:  lines.append(f"  Request:  {self.raw_request.hex(' ').upper()}")
        if self.raw_response: lines.append(f"  Response: {self.raw_response.hex(' ').upper()}")
        if self.parameters:   lines.append(f"  Parameters: {self.parameters}")
        if self.result:       lines.append(f"  Result: {self.result}")
        lines.append("-" * 70)
        return "\n".join(lines)
...
class ModbusRTUEmulator:
    ...
    def _process_frame(self, frame: bytes):
        ...
        if response:
            ...
            if operation:
                if "UNLOCKED" in operation: self.stats.locks_unlocked += operation.count("UNLOCKED")
                if "CLOSED" in operation:   self.stats.locks_locked += operation.count("CLOSED")
                if self.lock_state_callback: self.lock_state_callback(device_addr, operation)

            time.sleep(0.002)
            if self.serial_port: self.serial_port.write(response)
    ...
    def _log_raw_frame(self, frame: bytes, timestamp: str):
        ...
        names = {0x01:"Read Coils",0x02:"Read Discrete Inputs",0x03:"Read Holding Registers",0x04:"Read Input Registers",
                 0x05:"Write Single Coil",0x06:"Write Single Register",0x0F:"Write Multiple Coils",0x10:"Write Multiple Registers"}
        name = names.get(function_code & 0x7F, f"Unknown (0x{function_code:02X})")
        crc_ok = verify_crc(frame) if len(frame) >= 4 else False
        self.command_log_callback(CommandLog(timestamp, device_addr, function_code & 0x7F, name, frame, b'',
    ...
    def _log_raw_response(self, response: Optional[bytes], description: str, timestamp: str):
        if not self.command_log_callback: return
        if response:
            self.command_log_callback(CommandLog(timestamp, response[0] if len(response) else 0,
                                                 (response[1] & 0x7F) if len(response) > 1 else 0,
                                                 "Response", b'', response, description, f"Sent {len(response)} bytes"))
        else:
            self.command_log_callback(CommandLog(timestamp, 0, 0, "No Response", b'', b'', description, "No response sent"))
    ...
    def _log_command(self, request: bytes, response: bytes, function_code: int, device_addr: int, operation: Optional[str], timestamp: str):
        ...
        names = {0x01:"Read Coils",0x02:"Read Discrete Inputs",0x03:"Read Holding Registers",0x04:"Read Input Registers",
                 0x05:"Write Single Coil",0x06:"Write Single Register",0x0F:"Write Multiple Coils",0x10:"Write Multiple Registers"}
        name = names.get(function_code, f"Unknown (0x{function_code:02X})")
        params = ""; result = ""
        try:
            if function_code in [0x01, 0x02]:
                ...
                if len(response) > 3 and not (response[1] & 0x80):
                    bc = response[2]; data_bytes = response[3:3+bc]
                ...
                params = f"Start: 0x{sa:04X} ({sa}), Count: {cnt}"
                if len(response) > 3 and not (response[1] & 0x80):
                    bc = response[2]; result = f"Returned {bc // 2} registers ({bc} bytes)"
                else:
                    result = f"Exception: 0x{response[2]:02X}" if len(response) > 2 else "Exception"
            elif function_code == 0x05:
                a = struct.unpack('>H', request[2:4])[0]
                v = struct.unpack('>H', request[4:6])[0]
                st = "ON (Close/Lock)" if v == 0xFF00 else "OFF (Unlock)" if v == 0x0000 else f"Invalid (0x{v:04X})"
                ...
                params = "Unsupported function"; result = f"Exception: 0x{response[2]:02X}" if len(response) > 2 else "Error"
        except Exception as e:
            params = f"Error decoding: {e}"; result = "Parse error"

        self.command_log_callback(CommandLog(timestamp, device_addr, function_code, name, request, response, params, result))
...
class EmulatorGUI:
    def __init__(self, root):
        ...
        self._create_widgets(); self._refresh_ports()
    ...
    def _create_widgets(self):
        ...
        # CU48 zero-count compatibility toggle
        ...
        btn_frame = ttk.Frame(config_frame); btn_frame.grid(row=2, column=0, columnspan=5, padx=5, pady=5, sticky=tk.W)
        self.start_btn = ttk.Button(btn_frame, text="Start Emulator", command=self._start_emulator); self.start_btn.pack(side=tk.LEFT, padx=5)
        self.stop_btn = ttk.Button(btn_frame, text="Stop Emulator", command=self._stop_emulator, state=tk.DISABLED); self.stop_btn.pack(side=tk.LEFT, padx=5)

        status_frame = ttk.LabelFrame(self.root, text="Status", padding=10); status_frame.pack(fill=tk.X, padx=10, pady=5)
        self.status_label = ttk.Label(status_frame, text="Status: Stopped", foreground="red"); self.status_label.pack()

        content_frame = ttk.Frame(self.root); content_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)
    ...
    def _create_lock_controls(self):
        ...
        for i in range(48):
            ...
            setattr(self, f"_lock_btn_{i}", btn)
    ...
    def _update_lock_display(self):
        ...
        for i in range(48):
            btn = getattr(self, f"_lock_btn_{i}")
            if dev.coils[i]: btn.config(text="CLOSED", bg="green")
            else:            btn.config(text="UNLOCKED", bg="red")
    ...
    def _toggle_lock(self, lock_idx: int):
        ...
        dev = self.emulator.devices[device_id]
        dev.coils[lock_idx] = not dev.coils[lock_idx]; self._update_lock_display()
        self._log(f"Device {device_id} Lock #{lock_idx+1} manually set to {'CLOSED/LOCKED' if dev.coils[lock_idx] else 'UNLOCKED'}")

    def _close_all_locks(self):
        if not self.emulator: messagebox.showwarning("Warning", "Emulator not running"); return
        device_id = self.selected_device_var.get()
        if device_id not in self.emulator.devices: return
        dev = self.emulator.devices[device_id]; dev.coils = [True]*48; self._update_lock_display()
    ...
    def _unlock_all_locks(self):
        ...
        device_id = self.selected_device_var.get()
        if device_id not in self.emulator.devices: return
        dev = self.emulator.devices[device_id]; dev.coils = [False]*48; self._update_lock_display()
    ...
    def _on_command_received(self, command_log: 'CommandLog'):
        txt = command_log.format_log()
        self.command_log_text.insert(tk.END, txt + "\n"); self.command_log_text.see(tk.END)
    ...
    def _start_emulator(self):
        ...
        try:
            ...
            self._log(f"CU48 Emulator V3 (Fixed+) started: {port} @ {baudrate} baud; {mode}")
    ...
    def _save_statistics(self):
        ...
        ts = datetime.now().strftime("%Y%m%d_%H%M%S"); fn = f"modbus_cu48_stats_{ts}.txt"
        try:
            with open(fn, 'w') as f:
    ...
    def _save_command_log(self):
        ts = datetime.now().strftime("%Y%m%d_%H%M%S"); fn = f"modbus_cu48_commands_{ts}.log"
        try:
            content = self.command_log_text.get(1.0, tk.END)
