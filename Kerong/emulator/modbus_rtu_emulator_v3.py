...
Based on CU48 lock controller protocol specifications
- 48 locks per device (coils 0x00-0x2F)
- Default all locks closed (coil status = 1)
- Manual control of individual lock states
...
class ErrorDetail:
    ...
    def format_detailed(self) -> str:
        ...
        if len(self.frame) >= 2:
            ...
            }.get(func_code, f"Unknown/Invalid")
...
class CU48Device:
    ...
    def __init__(self, device_id: int):
        ...
        # Set default CU48 register values
        if device_id == 1:  # First device (address 0x01)
            self.holding_registers[0x0F] = 0xE230  # 57,904 (from original)
            self.holding_registers[0xF5] = 0x0002
            self.holding_registers[0xF6] = 0x0004

        # CU48-specific: Unlock time in milliseconds (default 550ms)
        self.holding_registers[0x03] = 550  # Unlock duration

    def get_lock_status_bytes(self) -> bytes:
        """Get 48 lock statuses as 6 bytes (48 bits)
        Returns bytes where each bit represents a lock (1=closed/locked, 0=open/unlocked)
        """
        result = bytearray(6)
        for i in range(48):
            byte_idx = i // 8
    ...
    def read_coils(self, start_addr: int, count: int) -> bytes:
        ...
        For CU48: Returns lock status (1=closed/locked, 0=open/unlocked)
        """
        # CU48 typically reads all 48 locks starting at address 0
    ...
    def write_single_coil(self, addr: int, value: int) -> Tuple[bytes, Optional[str]]:
        ...
        For CU48: Unlock/lock a single lock
        Returns: (response_bytes, operation_description)
        """
        if addr >= 48:
        ...
        operation = None
        if value == 0xFF00:
            # Set coil to 1 (close/lock the door)
            self.coils[addr] = True
            operation = f"LOCK #{addr+1} CLOSED"
        elif value == 0x0000:
            # Set coil to 0 (open/unlock the door)
    ...
    def write_multiple_coils(self, start_addr: int, count: int, values: bytes) -> Tuple[bytes, Optional[str]]:
        ...
        For CU48: Unlock/lock multiple locks
        Returns: (response_bytes, operation_description)
        """
        if start_addr >= 48 or start_addr + count > 48:
    ...
    def write_multiple_registers(self, start_addr: int, count: int, values: bytes) -> bytes:
        ...
        For CU48: Can be used to configure unlock time and other parameters
        """
        if start_addr + count > len(self.holding_registers):
...
class CommandLog:
    ...
    def format_log(self) -> str:
        ...
        # Always show parameters and result
        if self.parameters:
...
class ModbusRTUEmulator:
    ...
    def _process_frame(self, frame: bytes):
        ...
        if not verify_crc(frame):
            ...
            error = ErrorDetail(
                ...
                error_position=len(frame) - 2  # CRC starts at this position
            )
            self.stats.add_error(error)
            return
        ...
        if response:
            ...
            self._log_command(frame, response, function_code, device_addr, operation, timestamp)
            ...
            if response[1] & 0x80:  # Exception response
                if response[2] == 0x01:  # Illegal function
                    ...
                    # Create detailed error for unsupported function
                    error = ErrorDetail(
                        timestamp=timestamp,
                        error_type="UNSUPPORTED",
                        frame=frame,
                        description=f"Unsupported function code: 0x{function_code:02X}",
                        error_position=1  # Function code is at byte 1
                    )
                    self.stats.add_error(error)
            ...
            if self.serial_port:  # Only write if serial port is available
    ...
    def _log_raw_frame(self, frame: bytes, timestamp: str):
        ...
        # Parse basic info even if frame is invalid
        ...
        # Create simple log entry for raw frame
        log_entry = CommandLog(
            ...
            raw_request=frame,
            raw_response=b'',  # Will be filled later
    ...
    def _log_raw_response(self, response: Optional[bytes], description: str, timestamp: str):
        ...
        if response:
            log_entry = CommandLog(
                ...
                function_code=response[1] & 0x7F if len(response) > 1 else 0,  # Mask exception bit
    ...
    def _log_command(self, request: bytes, response: bytes, function_code: int, device_addr: int, operation: Optional[str], timestamp: str):
        ...
        # Decode parameters based on function code
        parameters = ""
        result = ""
...
class EmulatorGUI:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Modbus RTU Slave Emulator V3 - CU48 Protocol")
    ...
    def _on_lock_state_change(self, device_id: int, operation: str):
        ...
        # Update display if this is the currently selected device
    ...
    def _start_emulator(self):
        ...
        try:
            ...
            self._log(f"CU48 Emulator V3 started: {port} @ {baudrate} baud")
